...
router.get("/list", async (req, res) => {
    let {user_id, page, per_page, status} = req.query;
    let { data } = await axios.get(`${process.env.apiFormBuilder}/api/notification/list?&page=${page}&per_page=${per_page}&user_id=${user_id}&status=${status}`);
    // console.log(data)
    res.send(data);
});
