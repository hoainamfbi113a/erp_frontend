...
class TableRoles extends Component {
    ...
    for (let i = 0; i < this.state.dataPermission.length; i++) {
        ...
        if (dataRight[j].id == this.state.dataPermission[i].id) {
          ...
          for (const value of arrRight) {
            if (value.id === dataRight[j].id) {
              arrRightAction = lodash.intersectionBy(data.children,value.title,"title")
            }
          }
      ...
      for (let j = 0; j < dataRight.length; j++) {
        if (dataRight[j].id == this.state.dataPermission[i].id) {
          for (const value of arrRight) {
            if (value.id === dataRight[j].id) {
              const arrTemp = lodash.differenceBy(data.children,value.title,"title")
  ...
  showModal = (id) => {
    let role = this.state.data.filter((item) => {
      return item.id == id;
    });
    this.setState({
      id: role[0].id,
  ...
  handleChange = (targetKeys, direction, moveKeys) => {
    ...
    if(direction === "right"){
      ...
      axiosConfig.post(`/api/role/permission/${this.state.idRole}`,params)
      .then((res)=>{
        if(res.message === "Success!. Stored"){
      ...
      console.log(params);
      axiosConfig.post(`/api/role/permissiond/${this.state.idRole}`,params)
      .then((res)=>{
